...
export default function ViewMyChargeStations() {
  ...
  useEffect(() => {
    ...
    if (userCookie) {
      ...
      Swal.fire({
        ...
        text: "User session not found. Please login to access your chargers.",
        icon: "error",
        background: "#1f2937",
        color: "#f3f4f6"
      });
  ...
  const loadMyChargeStations = async () => {
    if (!userId) return;
    
    setLoading(true);
    try {
      const data = await fetchMyChargeStations(userId);
  ...
  const handleDelete = async (id) => {
    const confirm = await Swal.fire({
      ...
      confirmButtonText: "Remove Charger",
      cancelButtonText: "Keep Station",
      background: "#1f2937",
      color: "#f3f4f6"
    });
    ...
    try {
      ...
      Swal.fire({
        ...
        text: "Charging station deleted from network",
        icon: "success",
        background: "#1f2937",
        color: "#f3f4f6"
      });
  ...
  const handleStatusChange = async (id, isAvailable) => {
    try {
      ...
      Swal.fire({
        ...
        text: "Charger status changed successfully",
        icon: "success",
        background: "#1f2937",
        color: "#f3f4f6"
      });
  ...
  const handleEditSave = async () => {
    try {
      ...
      Swal.fire({
        ...
        text: "Charging station configuration updated successfully",
        icon: "success",
        background: "#1f2937",
        color: "#f3f4f6"
      });
  ...
  return (
    <DashboardLayout title="My Charging Infrastructure">
      {!userId ? (
        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-2xl p-8 text-center">
          <p className="text-yellow-400 text-lg font-semibold">Authentication required to access your charging infrastructure.</p>
        </div>
      ) : loading ? (
        <Loading text="Loading Your Charging Network..." />
        ...
        <div className="bg-gray-800 rounded-2xl shadow-2xl border border-purple-500/30 p-12 text-center">
          <div className="max-w-md mx-auto">
            ...
            <h3 className="text-2xl font-bold text-white mb-4">
              No Chargers Deployed
            </h3>
            <p className="text-gray-400 text-lg mb-8">
              You haven't deployed any charging stations yet. Launch your first charger to begin operations.
            </p>
          </div>
        </div>
      ) : (
            ...
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-white">
                  My Charging Network ({chargeStations.length})
                </h2>
                <p className="text-gray-400 text-lg mt-2">
                  Manage your charging infrastructure and operational status
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-8">
              ...
              <div className="space-y-4">
                <h3 className="text-xl font-bold text-white flex items-center space-x-3">
                  ...
                  <span>Update Charger Location</span>
                </h3>
