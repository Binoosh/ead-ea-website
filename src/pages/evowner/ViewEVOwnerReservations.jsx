...
export default function ViewEVOwnerReservations() {
  ...
  const [evOwnerId, setEvOwnerId] = useState(null);

  // Get EV Owner ID from cookies
  useEffect(() => {
    const ownerId = Cookies.get("evOwnerId");
    if (ownerId) {
      setEvOwnerId(ownerId);
    } else {
      Swal.fire({
        title: "Authentication Required",
        text: "EV Driver account not found. Please login to access your charging sessions.",
        icon: "error",
        background: "#1f2937",
        color: "#f3f4f6"
      });
  ...
  const loadReservations = async () => {
    ...
    try {
      ...
      // Filter reservations for current EV Owner
      const myReservations = data.filter(
        (reservation) => reservation.evOwner?.id === evOwnerId
      );
      setMyBookings(myReservations);
    } catch (err) {
      Swal.fire({
        title: "Error",
        text: err.message,
  ...
  const handleEditSave = async () => {
    try {
      ...
      Swal.fire({
        ...
        text: "Charging booking rescheduled successfully",
        icon: "success",
        background: "#1f2937",
        color: "#f3f4f6"
      });
  ...
  const getBookingStatus = (bookDate, fromTime) => {
    const bookingDateTime = new Date(`${bookDate.split("T")[0]}T${fromTime}`);
    const now = new Date();

    if (bookingDateTime > now) {
      return { status: "🟢 UPCOMING", color: "bg-green-500/20 text-green-400 border border-green-500/30" };
    } else {
      return { status: "⚫ COMPLETED", color: "bg-gray-500/20 text-gray-400 border border-gray-500/30" };
    }
  };

  return (
    <div className="flex justify-center min-h-screen bg-gradient-to-br from-gray-900 to-purple-900">
      <div className="max-w-7xl mx-auto p-8 w-full">
        {!evOwnerId ? (
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-2xl p-8 text-center">
            <p className="text-yellow-400 text-lg font-semibold">
              Authentication required to access your charging sessions.
            </p>
          </div>
        ) : loading ? (
          <Loading text="Loading Your Charging Sessions..." />
        ) : myBookings.length === 0 ? (
          <div className="bg-gray-800 rounded-2xl shadow-2xl border border-purple-500/30 p-12 text-center">
            <div className="max-w-6xl mx-auto">
              ...
              <p className="text-gray-400 text-lg mb-8">
                You haven't scheduled any charging sessions yet. Book your first session to power up your journey.
              </p>
            </div>
          </div>
        ) : (
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold text-white">
                    My Charging Sessions ({myBookings.length})
                  </h2>
                  <p className="text-gray-400 text-lg mt-2">
                    Manage and monitor your charging reservations
                  </p>
                </div>
              </div>
            </div>
                  ...
                  {myBookings.map((reservation) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-8 py-6 whitespace-nowrap">
                          <span
                            className={`px-4 py-2 rounded-xl text-sm font-bold ${bookingStatus.color}`}
                          >
                        ...
                        <td className="px-8 py-6 whitespace-nowrap text-lg font-bold">
                          <div className="flex items-center space-x-3">
                            ...
                            <button
                              ...
                              className="inline-flex items-center p-3 text-lg font-bold text-white bg-amber-600 rounded-xl hover:bg-amber-700 transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-amber-500 border border-amber-500/30"
